...
func TestSyncRuntimeStatus_DiscoversNewFromZellij(t *testing.T) {
	db, err := store.Open(":memory:")
	if err != nil {
		t.Fatal(err)
	}
	...
	zellijDumpLayout = func(sessionName string) (string, error) {
		return `layout {
    cwd "/Users/test/go/src/github.com/owner/repo"
    tab {
        pane command="claude"
    }
}`, nil
	}
	defer func() { zellijDumpLayout = origDump }()

	origRepo := gitRepoName
	origBranch := gitBranchName
	gitRepoName = func(cwd string) string { return "owner/repo" }
	gitBranchName = func(cwd string) string { return "feature/discovered" }
	...
	s := all[0]
	if s.ZellijSession != "new-session" {
		t.Errorf("zellij_session = %q, want new-session", s.ZellijSession)
	}
	...
	if s.GitBranch != "feature/discovered" {
		t.Errorf("git_branch = %q", s.GitBranch)
	}
	if s.Agent != "claude" {
