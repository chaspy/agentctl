...
type Action struct {
	ID             int64
	SessionID      string
	ActionType     string // send, receive, spawn, kill, note, decision, handoff
	Content        string
	Result         string
	RouteReason    string
...
func LogAction(db *sql.DB, a *Action) error {
	res, err := db.Exec(`INSERT INTO actions (session_id, action_type, content, result, route_reason, handoff_summary, token_burn)
		VALUES (?, ?, ?, ?, ?, ?, ?)`,
		a.SessionID, a.ActionType, a.Content, a.Result, a.RouteReason, a.HandoffSummary, a.TokenBurn)
	if err != nil {
		return err
	}
...
func GetRecentActions(db *sql.DB, limit int) ([]Action, error) {
	return queryActions(db, `SELECT id, session_id, action_type, content, result, route_reason, handoff_summary, token_burn, created_at
...
func queryActions(db *sql.DB, query string, args ...any) ([]Action, error) {
	...
	for rows.Next() {
		var a Action
		if err := rows.Scan(&a.ID, &a.SessionID, &a.ActionType, &a.Content, &a.Result, &a.RouteReason, &a.HandoffSummary, &a.TokenBurn, &a.CreatedAt); err != nil {
			return nil, err
		}
